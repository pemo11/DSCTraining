<#
 .Synopsis
 Install several apps with the process resource
#>

configuration InstallApps2
{
    node $AllNodes.NodeName
    {
        foreach($Product in $Node.ProductList)
        {
            ...
            WindowsProcess $PackageData.Name
            {
                Ensure = "Present"
...
$ConfigData = @{
    ...
    AllNodes = @(
        @{
            NodeName = "W2016X"
            ProductList = @("SevenZip")
        }
    )
}

InstallApps2 -ConfigurationData $ConfigData
